...
export function FeaturedProducts() {
  ...
  const handleAddToCart = async (productId: number) => {
    try {
      await ApiService.addToCart({ product_id: productId }) // Updated to object
          ...
          {products.map((product) => (
            <div key={product.id} className="group">
              <div className="relative overflow-hidden rounded-lg bg-muted aspect-square mb-3">
                <img
                  src={product.main_image || "/placeholder.svg?height=200&width=200&query=electronics"}
                  alt={product.name}
                  className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-300"
                />
                <Badge className="absolute top-2 right-2 bg-gradient-to-r from-primary to-secondary">Featured</Badge>
              </div>
